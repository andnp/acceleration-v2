import numpy as np
from src.agents.tdc.TDC import TDC

class NonlinearTDC(TDC):
    ...
    def value(self, s):
        return self.V.eval(s, self.theta[0][0])

    def computeGradient(self, s_t, a_t, s_tp1, r, gamma, p):
        w,h = self.theta[0][0], self.theta[1][0]
        ...
        dw = p * (delta * x_t - gamma * delta_hat * x_tp1)
